...
10. **For the DICOM use case, the remaining wins are config-level, not shader
    ...
    - **Min-max macrocell tuning.** Already active and now adaptive
      ...
      before shipping.
    - **Jitter: made much lighter with block-coherent IGN (−20% at sd 4,
      opt-in).** The jitter math is trivial; the cost is that a per-fragment
      ...
      pixel block (`SetJitterBlockSize`, opt-in; the legacy per-pixel default
      of 1 stays bit-identical to pre-block renders) restores lockstep
      marching while keeping the stochastic anti-aliasing: 800×800 sd 4 drops
      17.1 → 13.5 ms (−21%), 400×400 sd 4 9.2 → 8.0 ms (−13%), 400×400 sd 2
      ...
      coarser noise (sd-4 err 0.816 → 1.409), so 2×2 is the recommended opt-in.
      Block size 1 (default) restores the legacy per-pixel behavior.
