...
*Onboarding guide for `vtkMetalGPUVolumeRayCastMapper` + `MetalShaders.metal`. For perf deep-dives see `PerformanceInvestigation/perf_investigation_part2.md`, `PERFORMANCE_INVESTIGATION.md` and `PerformanceInvestigation/HARNESS_VS_APP_GAP.md` (previous comprehensive perf investigation — 42 sections, jitter gap `§1-6`, resolution `§7`, transposition `§26`, RG8 `§17`, compute marcher `§38`, fragment batch `§39`, mv9 coverage `§40` etc., `HARNESS_VS_APP_GAP.md:1` harness vs app; `perf_investigation_part2.md:7` now canonical for `M>GL`).*
...
See `CINEMATIC_METAL_PLAN.md` for cinematic, `VolumeRayCastPerfRegression.md` for history, `PerformanceInvestigation/HARNESS_VS_APP_GAP.md:1` for previous comprehensive investigation (42 sections — jitter, transposition, compute, `§39` batch, `§40` coverage etc.; jitter gap refuted, `perf_investigation_part2.md:7` now canonical for `M>GL`).
