...
fragment VolumeFragmentOut fragment_volume_main(
  ...
  // Only enabled when each coarse cell covers ≥ 4 voxels (volume ≥ 128³).
  ...
  // instruction-level parallelism.  When the coarse map is available,
  // empty cells are skipped before each sample by advancing currentPoint
  // past the cell boundary — no while-loop or continue needed.
  int i = 0;
  int maxStepsEven = maxSteps & ~1;
  for (; i < maxStepsEven; i += 2) {

    if (hasCoarseMap && (any(currentPoint < 0.0) || any(currentPoint > 1.0))) {
      break;
    }

    // --- Coarse map: skip empty space before sample 1 ---
    if (hasCoarseMap) {
    ...
    // --- Coarse map: skip empty space before sample 2 ---
    if (hasCoarseMap) {
      int3 curCell = clamp(int3(currentPoint / coarseCellSize), 0, 31);
      if (curCell.x != prevCell.x || curCell.y != prevCell.y || curCell.z != prevCell.z) {
        prevCell = curCell;
