...
// ---------------------------------------------------------------------------
// MetalFX Temporal Upscaling — Motion Vector Shaders
//
// Generates per-pixel motion vectors for temporal anti-aliasing and upscaling.
// For camera-only motion (most VTK use cases), motion is computed by comparing
// the current and previous frame's view-projection matrices.
//
// The motion vector format is RG16Float where:
//   R = horizontal motion in pixels (current - previous)
//   G = vertical motion in pixels (current - previous)
//
// With isJitteredMotionVectorsEnabled=YES on the temporal scaler, the jitter
// offset is automatically accounted for.
// ---------------------------------------------------------------------------

// Motion vector uniforms
struct MotionVectorUniforms {
  float4x4 inverseCurrentMVP; // inverse of current frame's MVP (for clip→world)
  float4x4 previousMVP;      // previous frame's MVP (for world→clip)
  float2 outputSize;          // output resolution (width, height)
  float2 renderSize;          // render resolution (width, height)
};

// Motion vector fragment output
struct MotionVectorOutput {
  float2 motion [[color(0)]];  // RG16Float motion vectors
};

// Fullscreen fragment shader that generates motion vectors from depth + MVP matrices.
// Reconstructs world position from depth using inverse current MVP, projects with
// previous MVP, and outputs the motion vector in pixel space.
fragment MotionVectorOutput fragment_motion_vector(
    FullscreenVertexOut in [[stage_in]],
    constant MotionVectorUniforms& mvUniforms [[buffer(0)]],
    texture2d<float> depthTexture [[texture(0)]],
    sampler depthSampler [[sampler(0)]]) {
  MotionVectorOutput out;

  // Sample depth at current pixel (Metal depth is [0,1])
  float depth = depthTexture.sample(depthSampler, in.texCoord).r;

  // Reconstruct clip-space position from texture coordinates and depth
  // texCoord is [0,1] with Y=0 at top (Metal convention)
  float2 ndc = float2(in.texCoord.x * 2.0 - 1.0,
                       in.texCoord.y * 2.0 - 1.0);
  float4 clipPos = float4(ndc, depth, 1.0);

  // Transform to world space using inverse of current MVP
  float4 worldPos = mvUniforms.inverseCurrentMVP * clipPos;

  // Project to previous frame's clip space
  float4 prevClipPos = mvUniforms.previousMVP * worldPos;

  // Previous frame NDC
  float2 prevNDC = prevClipPos.xy / prevClipPos.w;

  // Convert to pixel space motion vectors
  // Current pixel position in pixels (render resolution)
  float2 currentPixel = in.texCoord * mvUniforms.renderSize;
  // Previous pixel position in pixels (render resolution)
  float2 previousPixel = (prevNDC * 0.5 + 0.5) * mvUniforms.renderSize;

  // Motion vector = current - previous (in pixels)
  // MetalFX expects motion vectors in render-resolution pixel space
  out.motion = currentPixel - previousPixel;

  return out;
}

// Blit shader — copies a texture to the drawable for temporal upscale output.
// Used to copy the upscaled RGBA16Float result to the BGRA8Unorm drawable.
fragment float4 fragment_blit(
    FullscreenVertexOut in [[stage_in]],
    texture2d<float> sourceTexture [[texture(0)]],
    sampler blitSampler [[sampler(0)]]) {
  return sourceTexture.sample(blitSampler, in.texCoord);
}
