#!/bin/bash
#
# Re-runs the generic Metal image-comparison suite for Rendering/Volume and
# exports the failing renders (Metal + baseline + diff PNGs) plus a per-test
# metric manifest into a review folder, so failures can be inspected side by
# side.
#
# Volume suite mirror of export_image_compare.sh: same workflow, but scoped to
# the RenderingVolumeCxx-Metal tests and the Rendering/Volume baselines.
#
...
#
# Usage:
#   ./export_volume_image_compare.sh              # run suite, export + manifest
#   ./export_volume_image_compare.sh --no-run     # export from the last run's artifacts
#   ./export_volume_image_compare.sh --out /tmp/compare --jobs 4
#
# Options:
#   -b, --build-dir DIR   Metal build tree (default: <repo>/build_macos_metal)
#   -o, --out DIR         review output dir (default: <build>/Testing/ImageCompareReviewVolume)
...
OUT_DIR="${OUT_DIR:-${BUILD_DIR}/Testing/ImageCompareReviewVolume}"
mkdir -p "$OUT_DIR"
...
base_dir = os.path.join(build_dir, 'ExternalData', 'Rendering', 'Volume', 'Testing', 'Data', 'Baseline')
...
if os.path.exists(failed_list):
    with open(failed_list, errors='replace') as f:
        for line in f:
            line = line.strip()
            if line:
                failed_tests.append(line.split(':', 1)[-1].replace('VTK::RenderingVolumeCxx-Metal-', ''))
