...
- **Branch:** `metal-volume-parity-essential e05a147fb5` `6-fetch central` `thr fallback analytic 0.15` `y-transpose` default off for `512` fallback, `BASE` below default on for `512` real volumes → `b2e0286446` `fix(mv9): i>0 gate at Rendering/Metal/Shaders/MetalShaders.metal:5634` `thr 3.32→0.18` `VolumeRayCast`
...
- **Code map:** `Rendering/Metal/Shaders/MetalShaders.metal:4363` `maxSteps`, `:5634` `mv9 tEnd gate`, `:6416` `mv0 loop`, `:5568` `batchCap`, `:5600` `adaptive`, `:3861` `computeGradientFast`, `:5259` `MV9_COMPOSITE`, `:6254` `PROC_UNROLL_SAMPLE` `Rendering/Metal/vtkMetalGPUVolumeRayCastMapper.mm:242` `fc_marchVariant 9`, `:9632` `maxBatchWidth` `Rendering/Metal/Testing/Cxx/TestMetalVolumeRayCast.cxx:1` `parity` `Examples/GUI/iOSMetal/test-vtk-metal/NIFTIVolumeViewController.mm:42` `preset`
...
Cut: `maxStepsFrame=ceil(maxChord/SD)` `§4` already `uniform` → `batchCap=min(fragBatch, maxSteps/4)` `Rendering/Metal/Shaders/MetalShaders.metal:5600` `if(maxSteps<50) 2 else if<100 8 else 16` `→` `NIFTI→2` `DICOM→16` single `PSO` `f2 0.87` vs `f8 0.99` keeps `<1` both. Code: `Rendering/Metal/Shaders/MetalShaders.metal:5568` `batchCap`, `:5627 while(i<steps)` ladder.
...
```sh
# repro for far edge missing sliver (mv9 only) — before e05a147fb5 vs after b2e0286446
BIN=build_macos_metal/bin/vtkMetalGLVisualComparison
BASE="VTK_METAL_TEST_SAMPLE_DISTANCE=4 VTK_METAL_TEST_IMAGE_SAMPLE_DISTANCE=1.0 VTK_METAL_TEST_NUM_SLABS=1 VTK_METAL_TEST_IGN_JITTER=0 VTK_METAL_TEST_JITTER=1 VTK_METAL_TEST_MARCH_VARIANT=9 VTK_METAL_TEST_MINMAX=1 VTK_METAL_TEST_ACCEL=1 VTK_METAL_TEST_VOLTRANSPOSE_AXIS=y"
...
# code: Rendering/Metal/Shaders/MetalShaders.metal:5634, :4363, :6416, :5560
```
...
Next: §4 `batchCap=min(fragBatch,maxSteps/4)` adaptive `Rendering/Metal/Shaders/MetalShaders.metal:5600` `if(maxSteps<50)2 else if<100 8 else 16` → `NIFTI→2 DICOM→16` single `PSO` `f2 0.87 vs f8 0.99` keeps `<1` both (see §4). Code: `Rendering/Metal/Shaders/MetalShaders.metal:5568`, `:5627`, `:4600` `mmDimF`.
```
...
| unrolled variant | `Rendering/Metal/Shaders/MetalShaders.metal:6254` | `PROC_UNROLL_SAMPLE` |
...
| iOS preset | `Examples/GUI/iOSMetal/test-vtk-metal/NIFTIVolumeViewController.mm:42` | `Synthesized_FLASH25` |
