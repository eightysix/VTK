...
// Fragment compile-time batch (VTK_METAL_TEST_FRAG_BATCH): compile the ladder at a fixed width so dead
...
// 64 MB SegPool Private heaps that otherwise survive until
...
// Private-heap SegPool (64 MB) and the Compile-time PSO caches
// (SegBuild) that otherwise survive until
...
//  • Shared-storage fallback for SegPool (avoids Private-heap pressure
...
void vtkMetalGPUVolumeRayCastMapper::GPURender(vtkRenderer* ren, vtkVolume* vol)
{
  ...
  // Private-heap SegPool (64 MB) and the PSO caches that are
  ...
  // Gradient-based shading uniforms
  {
    ...
    // without it). Do not retune by sweep alone: the 0.125 optimum was
    // confounded by the 0.02 shading cull below.
    double cellSpacing[3];
    input->GetSpacing(cellSpacing);
    double avgSpacing =
      (fabs(cellSpacing[0]) + fabs(cellSpacing[1]) + fabs(cellSpacing[2])) / 3.0;
