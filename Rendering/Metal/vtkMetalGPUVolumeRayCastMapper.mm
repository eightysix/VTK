...
void vtkMetalGPUVolumeRayCastMapper::GPURender(vtkRenderer* ren, vtkVolume* vol)
{
  ...
  // PT: disk light for NEE — fixed above volume (top) for stable tests; onb variant is for final lateral key but top is stable for unit tests
  if (this->CinematicRendering && this->GetCinematicQuality() == vtkGPUVolumeRayCastMapper::PathTraced) {
    ...
    if (neeOff) {
      ...
      // Fixed top disk at (0.5,0.5,1.5) normal (0,0,-1) — stable for tests, lateral key is final product but top is testable
      lightUniforms.lights[0].position[0]=0.5f; lightUniforms.lights[0].position[1]=0.5f; lightUniforms.lights[0].position[2]=1.5f; lightUniforms.lights[0].position[3]=1.0f;
      lightUniforms.lights[0].direction[0]=0.0f; lightUniforms.lights[0].direction[1]=0.0f; lightUniforms.lights[0].direction[2]=-1.0f; lightUniforms.lights[0].direction[3]=0.0f;
      float rad = 30.0f;
