...
1. **Depth buffer occlusion** — When opaque geometry (e.g.骨骼 surface mesh)
...
2. **Near-plane clipping** — When the camera is inside the bounding box, the
   OpenGL path clips the box against the near plane. The Metal path renders
   the full box, wasting fragments behind the camera.

3. **Gradient-based shading** — Central-difference normals computed in the
   shader enable Phong lighting and gradient-opacity transfer functions. The
   Metal path has flat color-only compositing.

4. **Cropping regions** — Interactive 32-region crop without re-uploading data.

### Low priority gaps (specialized use cases)

5. Multi-volume compositing
6. Mask / label map support
7. Clipping planes
8. Volume partitioning (only needed for textures exceeding hardware 3D limit)
...
2. **Depth buffer occlusion** (Medium effort, Medium impact)
