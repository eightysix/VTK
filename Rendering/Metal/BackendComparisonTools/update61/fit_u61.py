...
def interp_seq(attrs, lam):
    t = [f32(lam[i]/w[i]) for i in range(3)]
    num = f32(f32(f32(t[0]*attrs[0]) + f32(t[1]*attrs[1])) + f32(t[2]*attrs[2]))
...
for (mx,my) in KNIFE:
    ...
    base = (f32((mx+0.5)/256.0-1.0), f32((511-my+0.5)/256.0-1.0))
    data.append((mx,my,gt,mtp,base))

def eval_offset(dx, dy, target):
    tot = 0
    for (mx,my,gt,mtp,base) in data:
        lam = bary_f32(ndc32, (base[0]+dx, base[1]+dy))
        for ax in range(3):
            attrs = [vtex[0][ax], vtex[1][ax], vtex[2][ax]]
            pred = interp_seq(attrs, lam)
            tot += abs(ulps(pred, (gt if target=='GL' else mtp)[ax]))
    return tot

# coarse scan in NDC units (1e-5 NDC ~ 0.0026 px)
best = None
for dx in np.arange(-8e-4, 8e-4, 2e-5):
    for dy in np.arange(-8e-4, 8e-4, 2e-5):
        e = eval_offset(float(dx), float(dy), 'GL')
        if best is None or e < best[0]:
            best = (e, dx, dy)
print('best GL offset: err=%d dx=%+.5e dy=%+.5e' % best)
best = None
for dx in np.arange(-8e-4, 8e-4, 2e-5):
    for dy in np.arange(-8e-4, 8e-4, 2e-5):
        e = eval_offset(float(dx), float(dy), 'Mt')
        if best is None or e < best[0]:
            best = (e, dx, dy)
print('best Mt offset: err=%d dx=%+.5e dy=%+.5e' % best)
# fine scan near best GL
bx, by = best[1], best[2]
for dx in np.arange(bx-1e-4, bx+1e-4, 2e-6):
    for dy in np.arange(by-1e-4, by+1e-4, 2e-6):
        e = eval_offset(float(dx), float(dy), 'GL')
        if best is None or e < best[0]:
            best = (e, dx, dy)
print('fine GL offset: err=%d dx=%+.5e dy=%+.5e' % best)
