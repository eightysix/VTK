...
/// near plane while the camera is inside the volume. The near plane is pulled
/// essentially onto the camera (clipping range near ~ 0), so the near-plane
/// clip is a no-op and the march starts at the eye.
...
static const char* TestGPURayCastCameraInsideTransformationNoShadeNoGradOpNoTransformNearPlaneTinyLog =
  "# StreamVersion 1\n"
  "EnterEvent 298 27 0 0 0 0 0\n"
  "MouseWheelForwardEvent 200 142 0 0 0 0 0\n"
  "LeaveEvent 311 71 0 0 0 0 0\n";

int TestGPURayCastCameraInsideTransformationNoShadeNoGradOpNoTransformNearPlaneTiny(
  int argc, char* argv[])
{
  ...
  // Pull the near plane essentially onto the camera so the near-plane clip is a
  // no-op (the march starts at the eye instead of at a clipped entry).
  ren->GetActiveCamera()->SetClippingRange(0.001, 400.0);
  ...
  int rv = vtkTesting::InteractorEventLoop(
    argc, argv, iren, TestGPURayCastCameraInsideTransformationNoShadeNoGradOpNoTransformNearPlaneTinyLog);
  return rv;
}
