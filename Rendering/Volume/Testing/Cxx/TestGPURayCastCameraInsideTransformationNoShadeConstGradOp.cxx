...
/// Description:
/// Variant of TestGPURayCastCameraInsideTransformationNoShade with the
/// gradient-opacity function constant (0.7 everywhere). The gradient magnitude
/// is still computed on the GPU but its value can no longer amplify the
/// per-sample opacity, so any remaining Metal/OpenGL divergence isolates the
/// composite path rather than the gradient-magnitude-to-gf lookup.
...
static const char* TestGPURayCastCameraInsideTransformationNoShadeConstGradOpLog =
  "# StreamVersion 1\n"
  "EnterEvent 298 27 0 0 0 0 0\n"
  "MouseWheelForwardEvent 200 142 0 0 0 0 0\n"
  "LeaveEvent 311 71 0 0 0 0 0\n";

int TestGPURayCastCameraInsideTransformationNoShadeConstGradOp(int argc, char* argv[])
{
  // Load data
  ...
  int rv = vtkTesting::InteractorEventLoop(
    argc, argv, iren, TestGPURayCastCameraInsideTransformationNoShadeConstGradOpLog);
  return rv;
}
